...
class ReviewService(ReviewRepository):
    ...
    @async_res_wrapper
    async def add_review(self, review_input: ReviewInput, book_service: "BookService") -> Review:
    ...
    def get_list_of_rating(self):
        return [
            str(rating_start) for rating_start in range(settings.MIN_REVIEW_RATING, settings.MAX_REVIEW_RATING+1)
        ]
