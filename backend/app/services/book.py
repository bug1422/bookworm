...
from app.models.book import BookQuery, Book, BookOutput, BookSearchOutput, BookDetailOutput, BookSortOption
...
class BookService():
    ...
    @async_res_wrapper
    async def get_recommended_books(self, limit: int = 8) -> list[BookSearchOutput]:
        books, _ = await self.repository.get_books(
    ...
    @async_res_wrapper
    async def get_popular_books(self, limit: int = 8) -> list[BookSearchOutput]:
        books, _ = await self.repository.get_books(
    ...
    @async_res_wrapper
    async def get_books(self, query_option: BookQuery) -> PagingResponse[BookSearchOutput]:
    ...
    @async_res_wrapper
    async def get_book_detail(self, book_id: int, review_service: ReviewService) -> BookDetailOutput:
    ...
    def __map_to_preview(self, book: Book, final_price: Decimal, total_review: int, avg_rating: float) -> BookOutput:
        return BookOutput(
    ...
    def __map_to_search_result(self, book: Book, discount_offset: Decimal, final_price: Decimal, total_review: int, avg_rating: float) -> BookSearchOutput:
    ...
    def __map_to_detail(self, book: Book, final_price: Decimal, total_review: int, avg_rating: float, review_count_by_rating) -> BookDetailOutput:
